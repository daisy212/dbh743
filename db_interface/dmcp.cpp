...
{
	uint32_t keybdata = 0;
	char result = OS_MAILBOX_Peek(&Mb_Keyboard, &keybdata);
	if ( 0 == result)
	{ // a key is here
		return 0;
	} else 
...
*/
{
	...
	char result = OS_MAILBOX_Get(&Mb_Keyboard, &keybdata);
	if ( 0 == result)
	{ // an event key is here, format db48 
		keybdata = keybdata & 0xff;
		if (keybdata>100) {
			return key_DB_to_DM(keybdata%100);
		} 
		else 
		{
			return -10; // 
...
//	OS_MAILBOX_Clear(&Mb_Keyboard);
	...
	while(1){
		char result = OS_MAILBOX_GetTimed(&Mb_Keyboard, &keybdata, 5000);
		if (result==0)
		{ // an envent keyboard occured...
			keybdata = keybdata & 0xff;
			if (keybdata<100) 		break;
