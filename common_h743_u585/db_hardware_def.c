...
#if  DBh743
   #define BKPSRAM_ADD (0x38800000)
   #define BKPSRAM_CLK_ON    __HAL_RCC_BKPRAM_CLK_ENABLE()
   #define BKPSRAM_CLK_OFF    __HAL_RCC_BKPRAM_CLK_DISABLE()
#elif DBu585
   #define BKPSRAM_ADD (0x40036400)
   #define BKPSRAM_CLK_ON    __HAL_RCC_BKPSRAM_CLK_ENABLE()
   #define BKPSRAM_CLK_OFF    __HAL_RCC_BKPSRAM_CLK_DISABLE()
#endif
...
// char * pstring =  (char*) (0x38800000 + read_adress*0x100);
   memcpy( read_data, (void *)(BKPSRAM_ADD + read_adress*0x100), siz);
...
void bkSRAM_WriteString(uint16_t read_adress, char* write_data, uint32_t length)
{
   ...
   BKPSRAM_CLK_ON;
// char * pstring =  (char*) (0x38800000 + read_adress*0x100);
   memcpy( (void*) (BKPSRAM_ADD + read_adress*0x100), write_data, siz);
   #if  DBh743
      SCB_CleanDCache_by_Addr((uint32_t *)(BKPSRAM_ADD + read_adress),256);
   #endif
   BKPSRAM_CLK_OFF;
   HAL_PWR_DisableBkUpAccess();
}
...
void bkSRAM_ReadVariable(uint16_t read_adress, uint32_t* read_data)
{
   HAL_PWR_EnableBkUpAccess();
   BKPSRAM_CLK_ON;
   *read_data =  *(uint32_t*) (BKPSRAM_ADD + read_adress);
   BKPSRAM_CLK_OFF;
   HAL_PWR_DisableBkUpAccess();
}
...
void bkSRAM_WriteVariable(uint16_t write_adress,uint32_t vall)
{
   HAL_PWR_EnableBkUpAccess();
   BKPSRAM_CLK_ON;
   *(__IO uint32_t*)(BKPSRAM_ADD + write_adress) = vall ;
   #if  DBh743
      SCB_CleanDCache_by_Addr((uint32_t *)(BKPSRAM_ADD + write_adress),8);
   #endif
   BKPSRAM_CLK_OFF;
   HAL_PWR_DisableBkUpAccess();
}
